...
const int kMAX_VELOCITY =
    6380 / 60 / 10 *
    2048;  // RPM->Convert to RPS->Convert to RP100MS->Convert to TP100MS
...
void DriveTrain::CartesianDrive(double y, double x, double rotation,
                                double angle, bool FOD) {
  // source: WPILib
  // same code found in CartesianDrive in the WPI Library but adapted for being
  // used in Velocity Mode
  ...
  Robot::GetRobot()
      ->GetCOB()
      .GetTable()
      .GetEntry(COB_KEY_DRIVE_MODE)
      .SetString(FOD ? "Field" : "Robot");

  if (FOD) {
    input.Rotate(angle);
  }

  x = std::abs(x) <= 0.05f ? 0 : x;
  y = std::abs(y) <= 0.05f ? 0 : y;
  rotation = std::abs(rotation) <= 0.025f ? 0 : rotation;
  ...
  m_FrontLeft.Set(ControlMode::Velocity,
                  wheelSpeeds[kFRONT_LEFT] * kMAX_VELOCITY);
  m_FrontRight.Set(ControlMode::Velocity,
                   wheelSpeeds[kFRONT_RIGHT] * kMAX_VELOCITY);
  m_BackLeft.Set(ControlMode::Velocity,
                 wheelSpeeds[kBACK_LEFT] * kMAX_VELOCITY);
  m_BackRight.Set(ControlMode::Velocity,
                  wheelSpeeds[kBACK_RIGHT] * kMAX_VELOCITY);

  Robot::GetRobot()
      ->GetCOB()
      .GetTable()
      .GetEntry(COB_KEY_MOTER_SPEED)
      .SetDouble(wheelSpeeds[kFRONT_LEFT]);
}
...
void DriveTrain::DriveInit() {
  ...
  //     m_FOD = !m_FOD;
  //     Robot::GetRobot()->GetCOB().GetTable().GetEntry(COB_KEY_DRIVE_MODE).SetString(m_FOD
  //     ? "Field" : "Robot");
  // });
